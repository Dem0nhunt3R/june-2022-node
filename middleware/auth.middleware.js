...
const {REFRESH_TOKEN} = require("../enum/tokenType");
const {FORGOT_PASSWORD} = require("../enum/tokenActionType");
...
module.exports = {
    ...
    checkRefreshToken: async (req, res, next) => {
        try {
            ...
            oauthService.verifyToken(refreshToken, REFRESH_TOKEN);
    ...
    checkActionToken: async (req, res, next) => {
        try {
            ...
            oauthService.verifyActionToken(actionToken, FORGOT_PASSWORD);
    ...
    checkOldPasswords: async (req, res, next) => {
        try {
            ...
            const results = await Promise.all(oldPasswords.map((record) => {
                return oauthService.compareOldPasswords(record.password, body.passwords);
            }));
